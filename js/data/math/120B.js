    ...
    "practice": `
        <div class="space-y-4 animate-fade-in" id="container-120B">
            <div class="flex flex-wrap justify-center gap-2 mb-4">
                <button onclick="window.switchTab120B('add')" class="tab-btn-120B active bg-blue-600 text-white px-6 py-2 rounded-full font-black shadow-lg">Cộng (+)</button>
                <button onclick="window.switchTab120B('sub')" class="tab-btn-120B bg-indigo-100 text-indigo-700 px-6 py-2 rounded-full font-black">Trừ (-)</button>
                <button onclick="window.switchTab120B('mul')" class="tab-btn-120B bg-purple-100 text-purple-700 px-6 py-2 rounded-full font-black">Nhân (×)</button>
                <button onclick="window.switchTab120B('div')" class="tab-btn-120B bg-amber-100 text-amber-700 px-6 py-2 rounded-full font-black">Chia (:)</button>
            </div>
            ...
            <div class="flex flex-col items-center pt-6 border-t border-dashed border-gray-100">
                 <button id="btn-submit-120B" onclick="window.submit120B()" class="btn-submit-global">Gửi bài làm</button>
            </div>
            ...
            <script>
                // Use a standard global object for lesson data to avoid scope issues
                window.problems120B = {
                    add: [
                ...
                window.renderGrids120B = function() {
                    ...
                    types.forEach(type => {
                        ...
                        if (!section) return;
                        section.innerHTML = '';
                        window.problems120B[type].forEach((p, i) => {
                            const card = document.createElement('div');
                            card.className = 'bg-white p-8 rounded-[40px] border-2 border-slate-100 shadow-xl hover:shadow-2xl transition-all duration-300';
                ...
                window.checkOne120B = function(type, i) {
                    const prob = window.problems120B[type][i];
                    let resStr = "";
                    if (type === 'mul') {
                        for(let c=1; c<=6; c++) {
                ...
                setTimeout(function() {
                    console.log("Initiating 120B Grids...");
                    if (typeof window.renderGrids120B === 'function') {
                        window.renderGrids120B();
                    }
                }, 300);
            </script>
        </div>
