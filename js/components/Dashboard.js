        ...
        return `
            <div class="touch-feedback flex items-center p-4 md:p-5 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm 
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <h3 class="text-sm md:text-base font-bold text-gray-800 dark:text-slate-200 leading-snug truncate">${lesson.title}</h3>
                </div>
        ...
        return `
            <div class="flex min-h-[calc(100vh-120px)] animate-fade-in">
                ...
                <aside class="hidden md:flex w-56 lg:w-64 flex-col shrink-0 pt-2 pr-0">
                    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden sticky top-28">
            ...
            return `
                ...
                <div class="flex-1 overflow-y-auto pb-24 md:pb-0">
                    <div class="bg-white dark:bg-slate-800/50 md:rounded-3xl md:shadow-sm md:border md:border-gray-100 dark:md:border-slate-700 p-4 md:p-8 min-h-full md:ml-4">
                ...
                : 'bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600 hover:text-gray-500'
            }">
                                    ${period}
                                </button>
                            `).join('')}
                        ...
                        <div class="mb-2 md:mb-4">
                            <nav class="text-[10px] font-bold text-blue-500 dark:text-blue-400 mb-1 md:mb-2 uppercase tracking-wider">
                                <span>${subject.name}</span> / <span class="text-gray-400 dark:text-slate-500">Tuần ${router.currentWeek}</span>
                            </nav>
                            <h2 class="text-2xl md:text-3xl font-extrabold text-gray-800 dark:text-white tracking-tight">${subject.name} - Tuần ${router.currentWeek}</h2>
                            <p class="text-sm text-gray-400 dark:text-slate-500 mt-1 md:hidden">Lớp 5 • Chương trình mới</p>
                        </div>

                        <!-- Week Selector (Horizontal Timeline) -->
                        <div class="border-y border-gray-200 dark:border-slate-700 my-4 md:my-8 py-3">
                ...
                return `
                                        <a onclick="router.selectDashboardWeek(${w})" 
                                           class="flex flex-col items-center shrink-0 py-1 cursor-pointer group transition-all ${isActive ? '' : 'opacity-40 hover:opacity-70'}">
                                            <span class="text-[9px] font-bold uppercase mb-1 ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500'}">Tuần</span>
                                            ${isActive
                        ? `<span class="flex items-center justify-center w-10 h-10 rounded-full bg-blue-600 text-white text-base font-black shadow-lg shadow-blue-200 dark:shadow-blue-900">${w}</span>`
                        : `<span class="text-sm font-bold text-gray-500 dark:text-slate-400">${w}</span>`
                    }
                                        </a>
                                    `;
            }).join('')}
                        ...
                        <!-- Progress Summary -->
                        <div class="mt-6 md:mt-8 bg-gray-50 dark:bg-slate-800 md:bg-white dark:md:bg-slate-800/80 rounded-2xl md:rounded-[32px] p-5 md:p-8 border border-gray-100 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                            ...
                            <div class="flex items-center gap-3 mb-4 md:mb-6 relative z-10">
                                <div class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white text-base shadow-lg shadow-blue-200 dark:shadow-blue-900">📊</div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 relative z-10">
                                <div class="bg-white dark:bg-slate-700/50 md:bg-gray-50/50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-50 dark:border-slate-600">
                                    <p class="text-[8px] text-gray-400 dark:text-slate-400 font-black uppercase tracking-widest mb-1">Số tiết học</p>
                                    <p class="text-xl font-black text-gray-800 dark:text-white">${lessons.length} tiết</p>
                                </div>
                                <div class="bg-white dark:bg-slate-700/50 md:bg-gray-50/50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-50 dark:border-slate-600">
                                    <p class="text-[8px] text-gray-400 dark:text-slate-400 font-black uppercase tracking-widest mb-1">Thời lượng</p>
                ...
                return `
                                            <div class="flex justify-between items-end mb-2">
                                                <p class="text-[8px] text-gray-400 dark:text-slate-400 font-black uppercase tracking-widest">Mục tiêu đã đạt</p>
                                                <p class="text-sm font-black text-blue-600">${percent}%</p>
                                            </div>
                                            <div class="w-full h-2.5 bg-gray-200 dark:bg-slate-600 rounded-full overflow-hidden">
